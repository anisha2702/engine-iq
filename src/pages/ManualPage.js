  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-10">
      ...
      <div className="mb-10">
        ...
        <h1 className="text-4xl font-black text-stone-900 mb-3">Understanding Your Engine Data</h1>
        ...
        <div>
          ...
          <div className="space-y-3">
            {filteredSensors.map((s) => (
              <div
                key={s.key}
                className="bg-transparent border border-[#44403c] rounded-xl overflow-hidden hover:border-stone-300 transition-colors"
              >
                <button
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-stone-400 font-mono">{s.key} · {s.unit}</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-3">
                    <span className="text-xs text-stone-400 hidden sm:block px-2 py-0.5 bg-stone-100 rounded-full">
                      {s.category}
                    </span>
                    <span className="text-stone-400 text-xs">{expandedSensor === s.key ? "▲" : "▼"}</span>
                  </div>
                </button>

                {expandedSensor === s.key && (
                  <div className="px-5 pb-5 border-t border-[#44403c]">
                    <p className="text-stone-700 text-sm mt-4 leading-relaxed mb-4">{s.tooltip}</p>
                    <div className="grid grid-cols-3 gap-3 text-xs">
                      <div className="bg-stone-100 rounded-lg p-3 text-center">
                        <p className="text-stone-400 mb-1">Minimum</p>
                        <p className="font-bold text-stone-900">{s.min} {s.unit}</p>
                      </div>
                      <div className="bg-amber-500/10 border border-amber-500/20 rounded-lg p-3 text-center">
                        <p className="text-stone-400 mb-1">Typical</p>
                        <p className="font-bold text-amber-400">{s.default} {s.unit}</p>
                      </div>
                      <div className="bg-stone-100 rounded-lg p-3 text-center">
                        <p className="text-stone-400 mb-1">Maximum</p>
                        <p className="font-bold text-stone-900">{s.max} {s.unit}</p>
                      </div>
                    </div>
                  </div>
                )}
      ...
      {activeSection === "howto" && (
        <div className="space-y-6">
          {howToMeasure.map(({ tool, icon, description, steps }) => (
            <div key={tool} className="bg-transparent border border-[#44403c] rounded-2xl p-6">
              <div className="flex items-start gap-4 mb-4">
                <span className="text-3xl">{icon}</span>
                <div>
                  <h3 className="font-bold text-stone-900 text-lg">{tool}</h3>
      ...
      {activeSection === "tips" && (
        <div className="space-y-6">
          {maintenanceTips.map(({ icon, title, tips }) => (
            <div key={title} className="bg-transparent border border-[#44403c] rounded-2xl p-6">
